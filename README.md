...
现实中的物体被光线照射时，会产生两种反射：漫反射和镜面反射。漫反射是光线射入物体后，在物体内部反复散射后再次射出的光线；镜面反射是不进入物体直接被反射的光线。注意，这与传统Phong渲染模型的概念完全不一样。一个物体粗糙与否并不影响它漫反射和镜面反射的比例。
在同样粗糙度的情况下：漫反射越多，物体显得越白、镜面反射越多，物体显得越光亮（例如，钻石和玻璃的表面粗糙度都非常低，但是钻石的IOR非常高，显得很闪耀。）
而金属则几乎全是镜面反射，没有漫反射。因此，相同粗糙度的金属和非金属看起来也是截然不同的：一块再光滑的石头也不能当镜子用。金属的IOR是一个复数，决定了金属的亮度和颜色。为了简便计算，在PBR Metallic模型中，金属的颜色贴图就可以用来表示金属的IOR，同时定义了一个金属度来决定一个物体是否是金属。
...
1. 因为原版光照计算采用的是古老的Phong，原版Diffuse贴图都是被调暗的。这点有老滚贴图制作经验的朋友应该深有体会。但PBR来说，我们只需要正确的，本质的颜色，所以要把贴图调亮到一个相对标准的亮度。这个看你自己。
...
6. AO贴图一般可以通过Materialize等工具生成，或者在Substance 3D Painter、Blender中烘焙得到。

7. 对于大多数材质来说，你可以使用一个全白的Specular贴图。只有部分材质需要注意，比如水、雪这种流体的反照率可能低于正常的材质，而钻石等宝石的反照率可能会很高。你可以在 https://physicallybased.info/ 查看一些常见材质的反照率。大多数不透明的非金属物体的这个值为0.04。水、冰、雪可以使用0.02。钻石可以使用0.16。如果你使用了纯白的Specular贴图，那么在Json中指定Specular的数值即可。
...
需要注意的是，specular_level是一个很重要的参数，真实的Specular值是这个参数乘Specular贴图的值。这个Specular的概念与上古卷轴原版的Specular概念，这个值是表示物体直接反射光线的量。当你的Specular贴图是全白的时候，这个参数就是最终的Specular值，即0.04，这也是绝大多数不透明非金属材质的Specular值。对于雪、水，最终的Specular值约为0.02，而钻石等宝石的Specular值可能会很高。金属材质的这个值无需关心，因为金属的IOR是复数并且由颜色表现。在PhysicallyBased.info上可以查到一些常见材质的Specular值，乘以0.08即可得到这个参数。
